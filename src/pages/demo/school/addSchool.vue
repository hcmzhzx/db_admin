   ...
   data () {
      return {
         ...
         rules: {
            ...
            contact: [{ required: true, message: '请输入联系方式', trigger: 'blur' }, { min: 11, max: 11, message: '请输入11位联系方式', trigger: 'blur' }],
   ...
   async created () {
      ...
      await httpGet('schoolopt', posts).then(res => {
         ...
         if (data) {
            ...
            this.form.did = this.district.find(item => item.id == data.did) ? data.did : ''
